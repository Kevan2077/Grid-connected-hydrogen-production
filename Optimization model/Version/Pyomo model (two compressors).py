...
def ECHO(year,location,step,grid,sell_ratio,num_interval,ratio,SO_k,batch_interval):
    #data import
    ...
    #Calendar month
    supply_periods=[0,24*31,24*28,24*31,24*30,24*31,24*30,24*31,24*31,24*30,24*31,24*30,24*31]
...
#######Define variables
    ...
    #Maximum power exchange between the grid and hydrogen production system
        #m.con_max_power_export=Constraint(expr=m.max_power_export==m.electrolyser_max_input)
    else:
        print("Off-Grid")
    ...
    if location=='TAS1':
        ...
        #m.electrolyser_max_input = Param(initialize=14400)
        m.con_grid_connection_fee=Constraint(expr=m.max_power_export*161==m.grid_connection_fee)
    if location=='SA1':
        print('Location: SA')
        #m.pv_capacity=Param(initialize=0*ratio)
        #m.wind_capacity=Param(initialize=18070*ratio)
        #m.h2_storage_capacity = Param(initialize=53107)
       # m.electrolyser_max_input = Param(initialize=15974)        #175kw
        m.con_grid_connection_fee=Constraint(expr=m.max_power_export*80==m.grid_connection_fee)
    ...
    if location=='VIC1':
        ...
        #m.electrolyser_max_input = Param(initialize=13001)
        m.con_grid_connection_fee=Constraint(expr=m.max_power_export*94==m.grid_connection_fee)
    ...
    if location=='NSW1':
        ...
        #m.electrolyser_max_input = Param(initialize=14775)
        m.con_grid_connection_fee=Constraint(expr=m.max_power_export*68==m.grid_connection_fee)
    ...
    m.H2CP_h2_storage= Var(m.time_periods, domain=NonPositiveReals)
    m.H2CP_h2_demand= Var(m.time_periods, domain=NonPositiveReals)       #export will be negative value

    #H2 storage node
    m.h2_storage_pin=Var(m.time_periods, domain=NonNegativeReals)
    m.h2_storage_pout=Var(m.time_periods, domain=NonPositiveReals)
...
####### Define the constraints  trials
    ...
    m.con_cost=Constraint(expr=m.grid_cost==sum(-m.grid_pout[i]*(m.price[i]+0.01)-(m.grid_pin[i]*(m.price[i]-0.034)) for i in m.time_periods))
...
##############Grid################
    ...
    def constraint_rule_comp(m, i):
        return  m.comp_pin[i]==0.41*m.h2_storage_pin[i]
    ...
    def constraint_rule_el(m, i):      #this indicates the power balance in different time steps for example if step is 15min the power consumed is 1/4 kWh
        return  m.el_pin[i]+m.el_pout[i]*(0.83+39.4/0.7)==0
    m.con_el = Constraint(m.time_periods, rule=constraint_rule_el)
    #0.83kWh/kg we required to compress hydrogen into the pipeline
