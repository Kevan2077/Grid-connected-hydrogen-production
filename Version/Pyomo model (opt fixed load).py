...
def ECHO(year, location, step, grid, max_power_ratio,sell_ratio, num_interval, ratio, SO_k, batch_interval):
    ...
    initial_values = {i: (3500 * 365 / number_of_supply) / 60 * step if i in m.supply_periods else 0 for i in
                      m.time_periods}
...
operation_result,key_indicators=ECHO(year=2021,location='QLD1',step=60,grid=1,max_power_ratio=0.1,sell_ratio=-1,num_interval=0,ratio=0,SO_k=0,batch_interval=24)
operation_result.to_csv(f'fixed batch 24.csv')
